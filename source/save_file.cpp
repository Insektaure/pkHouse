...
void SaveFile::setBoxSlot(int box, int slot, Pokemon pkm) {
    ...
    // DEBUG: capture post-checksum state for round-trip verification
    // (getEncrypted calls refreshChecksum on pkm.data before encrypting to outBuf,
    //  so pkm.data now has the correct checksum but is still decrypted)
    uint8_t preEncrypt[PokeCrypto::MAX_PARTY_SIZE];
    std::memcpy(preEncrypt, pkm.data.data(), sizeof(preEncrypt));
    uint16_t preSpecies = preEncrypt[0x08] | (preEncrypt[0x09] << 8);
    uint16_t preSanity  = preEncrypt[0x04] | (preEncrypt[0x05] << 8);

    // DEBUG: round-trip verification — decrypt what we just wrote and compare
    {
        int dataSize = sizeBoxSlot_ - gapBoxSlot_;
        Pokemon verify;
        verify.gameType_ = gameType_;
        verify.loadFromEncrypted(boxData_ + offset, dataSize);

        uint16_t postSpecies = verify.data[0x08] | (verify.data[0x09] << 8);
        uint16_t postSanity  = verify.data[0x04] | (verify.data[0x05] << 8);
        uint16_t postCheck   = verify.data[0x06] | (verify.data[0x07] << 8);
        uint16_t preCheck    = preEncrypt[0x06] | (preEncrypt[0x07] << 8);

        bool mismatch = false;
        for (int i = 0; i < dataSize; i++) {
            if (verify.data[i] != preEncrypt[i]) {
                mismatch = true;
                break;
            }
        }

        // Always dump the decrypted PA8 for PLA (so user can open in PKHeX)
        if (gameType_ == GameType::LA) {
            std::string pa8Path = "/switch/pkHouse/debug_last_pa8.pa8";
            FILE* pa8f = std::fopen(pa8Path.c_str(), "wb");
            if (pa8f) {
                std::fwrite(preEncrypt, 1, PokeCrypto::SIZE_8ASTORED, pa8f);
                std::fclose(pa8f);
            }
        }

        if (mismatch || postSpecies != preSpecies) {
            // Write debug dump to SD card
            std::string dbgPath = "/switch/pkHouse/debug_roundtrip.bin";
            FILE* dbg = std::fopen(dbgPath.c_str(), "wb");
            if (dbg) {
                // Write: [4 bytes "PRE\0"][dataSize bytes pre-encrypt][4 bytes "PST\0"][dataSize bytes post-decrypt]
                const char hdrPre[] = "PRE";
                const char hdrPst[] = "PST";
                std::fwrite(hdrPre, 1, 4, dbg);
                std::fwrite(preEncrypt, 1, dataSize, dbg);
                std::fwrite(hdrPst, 1, 4, dbg);
                std::fwrite(verify.data.data(), 1, dataSize, dbg);
                std::fclose(dbg);
            }
            // Also write a human-readable summary
            std::string txtPath = "/switch/pkHouse/debug_roundtrip.txt";
            FILE* txt = std::fopen(txtPath.c_str(), "a");
            if (txt) {
                std::fprintf(txt, "ROUNDTRIP MISMATCH box=%d slot=%d game=%d\n", box, slot, (int)gameType_);
                std::fprintf(txt, "  Pre:  species=%u sanity=%u checksum=%u EC=%08X\n",
                    preSpecies, preSanity, preCheck,
                    (unsigned)(preEncrypt[0]|(preEncrypt[1]<<8)|(preEncrypt[2]<<16)|(preEncrypt[3]<<24)));
                std::fprintf(txt, "  Post: species=%u sanity=%u checksum=%u EC=%08X\n",
                    postSpecies, postSanity, postCheck,
                    (unsigned)(verify.data[0]|(verify.data[1]<<8)|(verify.data[2]<<16)|(verify.data[3]<<24)));
                // Find first differing byte
                for (int i = 0; i < dataSize; i++) {
                    if (verify.data[i] != preEncrypt[i]) {
                        std::fprintf(txt, "  First diff at byte 0x%03X: pre=0x%02X post=0x%02X\n",
                            i, preEncrypt[i], verify.data[i]);
                        break;
                    }
                }
                std::fclose(txt);
