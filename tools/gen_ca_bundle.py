...
every Let's Encrypt chain ends at one of two roots. Two certificates is 2.7 KB
and parses in a blink.

    python3 tools/gen_ca_bundle.py            # Let's Encrypt only (default)
    python3 tools/gen_ca_bundle.py --full     # the machine's whole trust store

THE TRADE: the minimal bundle trusts Let's Encrypt and nothing else. Put the
...
def main() -> None:
    ...
    if args.full:
        ...
        pems = [find_root(name) for name in LETSENCRYPT_ROOTS]
        listed = [describe(p) for p in pems]
        body = b"".join(pems)
        certs = len(pems)
        lines = [
            "# pkHouse trust store -- Let's Encrypt only.",
            "# GENERATED by tools/gen_ca_bundle.py. Do not edit; regenerate.",
            "#",
            "# Every Let's Encrypt chain ends at one of these two roots, and the",
            "# server sends the intermediate itself, so this is all a client",
            "# needs to verify a board on OVH's free certificate.",
            "#",
            "# This trusts Let's Encrypt and NOTHING ELSE. A board behind",
